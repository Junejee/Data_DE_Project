...
GCS_BUCKET = "xxxxxxxxxxx"
BIGQUERY_DATASET = "networkrail"
KEYFILE_FOR_GCS = "/opt/airflow/dags/Service_Account_to_upload_GCS.json"
KEYFILE_FOR_GCS_TO_BIGQUERY = "/opt/airflow/dags/deb-loading-data-to-db.json"

def _load_data_from_gcs_to_bigquery(data_interval_start, **context):
    ds = data_interval_start.to_date_string()

    keyfile_bigquery = f"{DAGS_FOLDER}/deb-loading-data-to-db.json"
    ...
    print(f"Loaded {table.num_rows} rows and {len(table.schema)} columns to {table_id}")    # Your code here
...
# Transform data in data lake using Spark
    ...
    transform_data = EmptyOperator(task_id="transform")
